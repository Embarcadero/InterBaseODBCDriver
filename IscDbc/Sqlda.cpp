 ...
 *                INTB-1903: Changes for InterBase ODBC driver
 *                  In getPrecision(), for dialect 3 databases, return sqlprecision from sqlvar for NUMERIC/DECIMAL subtypes.  
 * 
 *   05-Sep-2013: Contribution by Embarcadero Technologies Inc.
 *                INTB-1824: Changes for InterBase ODBC driver
...
void Sqlda::print()
{
	...
	for (int n = 0; n < sqlda->sqld; ++n, ++var)
		{
		char *p = var->sqldata;
		printf ("%d. type %d, len %d, precision %d, addr %p (%p) ",
				n, var->sqltype, var->sqllen, var->sqlprecision, p, var->sqlind);
...
// Return SQLDA for all system field has (METADATALENGTH -1) length 
...
/* For dialect 3 databases and above, sqlprecision is returned for NUMERIC/DECIMAL subtypes */
	switch (var->sqltype & ~1)
	{
	case SQL_TEXT:
		if ( var->sqllen == 1 && var->sqlsubtype == 1 )
	...
	case SQL_SHORT:
		return (var->sqlprecision) ? var->sqlprecision : SET_INFO_FROM_SUBTYPE(	MAX_NUMERIC_SHORT_LENGTH,
										MAX_DECIMAL_SHORT_LENGTH,
										MAX_SMALLINT_LENGTH);

	case SQL_LONG:
		return (var->sqlprecision) ? var->sqlprecision : SET_INFO_FROM_SUBTYPE(	MAX_NUMERIC_LONG_LENGTH,
	...
	case SQL_QUAD:
	case SQL_INT64:
		return (var->sqlprecision) ? var->sqlprecision : SET_INFO_FROM_SUBTYPE(	MAX_NUMERIC_LENGTH,
										MAX_DECIMAL_LENGTH,
										MAX_QUAD_LENGTH);
...
{
	return *(short*)Var(index)->sqlind < 0;
}
