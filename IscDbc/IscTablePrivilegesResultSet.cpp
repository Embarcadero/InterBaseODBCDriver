...
void IscTablePrivilegesResultSet::getTablePrivileges(const char * catalog, const char * schemaPattern, const char * tableNamePattern)
{
	char sql[2048] =  "select cast ('' as varchar(7)) as table_cat,"					//1
				          "cast (tbl.rdb$owner_name as varchar(67)) as table_schem,"	//2
						  "cast (tbl.rdb$relation_name as varchar(67)) as table_name,"	//3
						  "cast (priv.rdb$grantor as varchar(67)) as grantor,"			//4
						  "cast (priv.rdb$user as varchar(67)) as grantee,"				//5
