...
#              Cloned install.sh script from Linux, and enabled support for macOS
#
...
CheckEnvironment()
{
    ...
    if [ -z "$fbODBCDOCS" ]; then
        fbODBCDOCS="/usr/local/share/doc"
    fi
...
if [ "$TEST" = "1" ]; then
    ...
    if [ -d /Applications/InterBase_ib2020 ]; then
