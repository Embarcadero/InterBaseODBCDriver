...
BOOL CDsnDialog::OnFindFile()
{
	...
	char * szOpenFilter =	"InterBase Database Files (*.ib;*.gdb)\0*.ib;*.gdb\0"
							"All files (*.*)\0*.*\0"
							"\0";
    ...
    ofn.lpstrTitle = _TR( IDS_DLG_TITLE_FINDFILE_DATABASE, "Select InterBase database" );
...
BOOL CDsnDialog::OnFindFileClient()
{
    ...
    char strFullPathFileName[256];
    char achPathFileName[256];
	char * szOpenFilter =	"InterBase Client Files (*.dll)\0*.dll\0"
							"All files (*.*)\0*.*\0"
							"\0";
    ...
    ofn.nMaxFileTitle = sizeof(achPathFileName);
    ofn.lpstrInitialDir = NULL;
    ofn.lpstrTitle = _TR( IDS_DLG_TITLE_FINDFILE_CLIENT, "Select InterBase client" );
    ofn.Flags = OFN_FILEMUSTEXIST | OFN_HIDEREADONLY | OFN_PATHMUSTEXIST;
    ofn.lpstrDefExt = "*.dll";
    ofn.nFileOffset = 0;
    ofn.nFileExtension = 0;
...
void ProcessCDError(DWORD dwErrorCode, HWND hWnd)
{
	...
	MessageBox( hWnd, stringID, TEXT( _TR( IDS_DLG_TITLE_SETUP, "InterBase ODBC Setup" ) ), MB_OK );
}
...
intptr_t CDsnDialog::DoModal()
{
	...
	*p++ = 0;          // Class
	...
	nchar = nCopyAnsiToWideChar (p, TEXT( _TR( IDS_DLG_TITLE_SETUP, "InterBase ODBC Setup" ) ) );
	p += nchar;
