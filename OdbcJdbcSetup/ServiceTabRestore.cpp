 ...
 *              InterBase supports 1/2/4/8/16KB page size only
 *              Default restore page size is 4KB for InterBase
 *
 *
    ...
    case WM_INITDIALOG:
		{
						  ...
						  SWP_NOSIZE | SWP_NOZORDER );
			{
				...
				SendMessage( hWndBox, CB_SETCURSEL, 2, 0 ); // 4096
			}
...
{
	char * szCaption    = "Select InterBase backup file";
	char * szOpenFilter = "InterBase Backup Files (*.ibk;*.gbk)\0*.ibk;*.gbk\0"
                          "All files (*.*)\0*.*\0"
                          "\0";
	char * szDefExt     = "*.gbk";
...
bool CServiceTabRestore::createDialogIndirect( CServiceTabCtrl *parentTabCtrl )
{
	...
	if ( backupPathFile.IsEmpty() )
		setDefaultName( "gbk", backupPathFile );
