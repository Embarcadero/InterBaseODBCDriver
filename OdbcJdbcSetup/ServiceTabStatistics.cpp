 ...
 *  All Rights Reserved.
 *
 * 10-Aug-2012: Contribution by Embarcadero Technologies Inc.
 *      Changes for InterBase ODBC driver
 *      "Statistics" dialog changes to show "Log page" and "Show interbase.log"
 *
 */
...
void CServiceTabStatistics::onStartStatistics()
{
	...
	catch ( std::exception &ex )
	{
		...
		sprintf(buffer, "sqlcode %d, ibcode %d - %s", exception.getSqlcode(), exception.getFbcode(), (const char*)text );
...
bool CServiceTabStatistics::buildDlgChild( HWND hWndParent )
{
	...
	*p++ = 0;          // Class
	...
	nchar = nCopyAnsiToWideChar( p, TEXT( _TR( IDS_DLG_TITLE_SETUP, "InterBase ODBC Service" ) ) );
	p += nchar;
	...
	*p++ = 8;          // FontSize
    ...
    TMP_RADIOCONTROL  ( "Log page",IDC_RADIO_DATABASE_LOG, "Button",BS_AUTOCHECKBOX | WS_TABSTOP,185,113,130,10 )
    TMP_RADIOCONTROL  ( "Show interbase.log",IDC_RADIO_SHOW_DATABASE_LOG, "Button",BS_AUTOCHECKBOX | WS_TABSTOP,185,129,130,10 )
